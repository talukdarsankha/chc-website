...
//     exit();
//   } else {
    $target_dir = "images/broadcast-banners/";
    ...
    if ($uploadOk == 0) {
      ...
      if (move_uploaded_file($_FILES["image"]["tmp_name"],"../".$target_file)) {
        $data = [
          ...
          'broadcast_banner'=>$target_file,
          'broadcast_by'=>$broadcastBy,
          'broadcast_date'=>$broadcastDate
        //   'added_time'=>$time
        ];

        $create = $crud->Create($data,"broadcast");

        if($create) {
            $data['successMessage'] = "Broadcast Added Successfully";
